...
{ 
    ...
    if ($row['row'] == 0) { 
	    ...
	    $sql = "INSERT INTO STOCKIN(product_name, reciept_date, stock_summary, receipt_no, stock_in, balance_left, remarks, recordedBy, recordedOn) VALUES('$prodname[0]','$date','$summary','$receiptno','$stockin','$balance','$remarks', '".$_SESSION['name']."', '".date("Y-m-d H:i:s")."')";  
...
{ 
    ...
    $fields = array('产品名称/PRODUCT NAME (e.g 铅笔)', '收据日期/RECEIPT DATE(e.g 20/01/1234)', '摘要/STOCK SUMMARY(e.g any)', '收据编号/RECEIPT NO(e.g 123)', '总数(进货)/STOCK IN(e.g 10)', '结存/BALANCE LEFT(e.g 10)' ,'备注/REMARKS');   
    $fileName = "stockin_template_" . date('Y-m-d') . ".xlsx"; 
