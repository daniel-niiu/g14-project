    ...
    if(in_array($file_extension, $allowed_extension))
    {
        ...
        {  
            if($i!=0){   
                ...
                $query="SELECT product_name,reciept_date,receipt_no FROM stockin WHERE product_name = '".$row[0]."' AND reciept_date = '".$date."' AND receipt_no = '".$row[3]."'"; 
        ...
        if(!empty($non_empty_array))
        {
            ...
            header("Location: stock-in.php?name=stock&aside=stock-in&lang=".$_SESSION['lang']."&import_status=fail");
        }
        else
        {
            foreach($data as $row)
